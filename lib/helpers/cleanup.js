...
function cleanupSvg(svg, keepFillColor) {
  var extractedWidth = arguments.length <= 2 || arguments[2] === undefined ? 0 : arguments[2];
  var extractedHeight = arguments.length <= 3 || arguments[3] === undefined ? 0 : arguments[3];

  var height = extractedHeight ? 'typeof height === "number" ? height : (' + extractedHeight + ')' : 'typeof height === "number" ? height : size || 0';
  var width = extractedWidth ? 'typeof width === "number" ? width : (' + extractedWidth + ')' : 'typeof width === "number" ? width : size || 0)';
  var cleanedSvg = _basicCleanup(svg).replace(/viewBox/, '{...rest} height={' + height + '} width={' + width + '} onClick={onClick} style={style} className={className} viewBox');
