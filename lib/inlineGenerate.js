...
var _templateIconTsxNunjucks = require('../template/icon-tsx.nunjucks');

var _templateIconTsxNunjucks2 = _interopRequireDefault(_templateIconTsxNunjucks);
...
function generateInline(config, svgFiles) {
  ...
  return _bluebird2['default'].all(fileReaders).then(function (svgs) {
    ...
    return _bluebird2['default'].all(svgPromises).then(function (results) {
      ...
      return {
        icons: icons,
        source: _templateIconTsxNunjucks2['default'].render({ icons: icons, comment: comment, reactPureRender: reactPureRender })
      };
    });
  });
}
