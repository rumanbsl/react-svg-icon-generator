  ...
  return function () {
    ...
    Promise.all(svgPromises).then(function (results) {
      var icons = results.map(function (result) {
        var extractedWidth = result.svg.data.substring(result.svg.data.indexOf("<svg"), result.svg.data.indexOf(">")).match(/width=\"?\'?[0-9]+[A-Za-z0-9]+?"/)[0].replace("width=\"", "").replace("\"", "");
        var extractedHeight = result.svg.data.substring(result.svg.data.indexOf("<svg"), result.svg.data.indexOf(">")).match(/height=\"?\'?[0-9]+[A-Za-z0-9]+?"/)[0].replace("height=\"", "").replace("\"", "");
