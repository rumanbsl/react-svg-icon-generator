...
function configureGenerator(config) {
  return function () {
    ...
    var defaultTemplate = (function (c) {
      if (c.native) {
        if (c.tsx) {
          return _path2['default'].join(__dirname, '..', 'template', 'icon_native.tsx.nunjucks');
        } else {
          return _path2['default'].join(__dirname, '..', 'template', 'icon_native.nunjucks');
        }
      } else {
        if (c.tsx) {
          return _path2['default'].join(__dirname, '..', 'template', 'icon-tsx.nunjucks');
        } else {
          return _path2['default'].join(__dirname, '..', 'template', 'icon.nunjucks');
        }
      }
    })(config);
    ...
    Promise.all(svgPromises).then(function (results) {
      var icons = results.map(function (result) {
        var extractedWidth = (function () {
          var arr = result.svg.data.substring(result.svg.data.indexOf("<svg"), result.svg.data.indexOf(">")).match(/width=\"?\'?[0-9]+[A-Za-z0-9]+?"/);
          if (Array.isArray(arr) && arr.length > 0) {
            return arr[0].replace("width=\"", "").replace("\"", "");
          }
          return "";
        })();
        var extractedHeight = (function () {
          var arr = result.svg.data.substring(result.svg.data.indexOf("<svg"), result.svg.data.indexOf(">")).match(/height=\"?\'?[0-9]+[A-Za-z0-9]+?"/);
          if (Array.isArray(arr) && arr.length > 0) {
            return arr[0].replace("height=\"", "").replace("\"", "");
          }
          return "";
        })();
        console.log({ extractedHeight: extractedHeight, extractedWidth: extractedWidth });
        return {
